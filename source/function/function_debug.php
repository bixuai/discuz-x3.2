...
function debugmessage($ajax = 0) {
	...
	foreach ($db->sqldebug as $string) {
		...
		$sqldebug .= '<li><span style="cursor:pointer" onclick="document.getElementById(\'sql_'.$n.'\').style.display = document.getElementById(\'sql_'.$n.'\').style.display == \'\' ? \'none\' : \'\'">'.$string[1].'s &bull; DBLink '.$links[md5(serialize($string[3]))].$dt.'<br />'.$sql.'</span><br /></li>'.$sqldebugrow;
	}
